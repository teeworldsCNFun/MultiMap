...
CGrowingExplosion::CGrowingExplosion(CGameWorld *pGameWorld, vec2 Pos, vec2 Dir, int Owner, int Radius, int ExplosionEffect, int MapID)
		: CEntity(pGameWorld, CGameWorld::ENTTYPE_GROWINGEXPLOSION, MapID),
		m_pGrowingMap(NULL),
		m_pGrowingMapVec(NULL)
{
	...
	if(GameServer()->Collision(GetMapID())->CheckPoint(explosionTile) && length(Dir) <= 1.1)
	{
	...
	for(int j=0; j<m_GrowingMap_Length; j++)
	{
		for(int i=0; i<m_GrowingMap_Length; i++)
		{
			...
			if(GameServer()->Collision(GetMapID())->CheckPoint(Tile) || distance(Tile, m_SeedPos) > m_MaxGrowing*32.0f)
			{
	...
	switch(m_ExplosionEffect)
	{
		case GROWINGEXPLOSIONEFFECT_FREEZE:
			if(random_prob(0.1f))
			{
				GameServer()->CreateHammerHit(m_SeedPos, GetMapID());
			}
			break;
		case GROWINGEXPLOSIONEFFECT_POISON:
			if(random_prob(0.1f))
			{
				GameServer()->CreateDeath(m_SeedPos, m_Owner, GetMapID());
			}
			break;
				...
				if(FromLeft || FromRight || FromTop || FromBottom)
				{
					...
					switch(m_ExplosionEffect)
					{
						case GROWINGEXPLOSIONEFFECT_FREEZE:
							if(random_prob(0.1f))
							{
								GameServer()->CreateHammerHit(TileCenter, GetMapID());
							}
							break;
						case GROWINGEXPLOSIONEFFECT_POISON:
							if(random_prob(0.1f))
							{
								GameServer()->CreateDeath(TileCenter, m_Owner, GetMapID());
							}
							break;
						case GROWINGEXPLOSIONEFFECT_LOVE:
							if(random_prob(0.2f))
							{
								GameServer()->CreateLoveEvent(TileCenter, GetMapID());
							}
							break;
						case GROWINGEXPLOSIONEFFECT_HIT_EFFECT:
							GameServer()->CreateExplosion(TileCenter, m_Owner, WEAPON_HAMMER, false, GetMapID());
							break;
						...
						case GROWINGEXPLOSIONEFFECT_ELECTRIC:
							{
								...
								if(NumPossibleStartPoint > 0)
								{
									...
									GameServer()->CreateLaserDotEvent(StartPoint, EndPoint, Server()->TickSpeed()/6, GetMapID());
								}
								
								if(random_prob(0.1f))
								{
									GameServer()->CreateSound(EndPoint, SOUND_RIFLE_BOUNCE, GetMapID(), GetMapID());
								}
							}
							break;
					}
			...
			if(tick - m_pGrowingMap[k] < Server()->TickSpeed()/4)
			{
				switch(m_ExplosionEffect)
				{
					case GROWINGEXPLOSIONEFFECT_FREEZE:
						...
						break;
					case GROWINGEXPLOSIONEFFECT_POISON:
						p->Poison(g_Config.m_PoisonDmg, m_Owner);
